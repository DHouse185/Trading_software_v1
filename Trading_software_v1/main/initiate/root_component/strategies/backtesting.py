...
class BackTest:
    ...
    def take_profit_calc(self, entry_price=None, side=None) -> float:
        ...
        if side == 1:
            take_profit = round(float(entry_price + (entry_price * (self.take_profit / 100))), 8)
            return take_profit
    ...
    def stop_loss_cal(self, entry_price=None, side=None) -> float:
        ...
        if side == 1:
            stop_loss = round(float(entry_price - (entry_price * (self.stop_loss / 100))), 8)
            return stop_loss
    ...
    def enter_exit_trade(self):
        ...
        for idx, i in enumerate(self.testing_dataframe.index):
            ...
            and (stop_loss_value in np.arange(self.testing_dataframe.loc[i, 'low'], self.testing_dataframe.loc[i, 'high'], 0.0001)
            ...
            in np.arange(self.testing_dataframe.loc[i, 'low'], self.testing_dataframe.loc[i, 'high'], 0.0001)
...
class TraditionalIchimokuStrategy(BackTest):
    ...
    def apply_signal(self):
        ...
        non_shift_senkou_span_a = senkou_span_a.shift(-self.chikou_senkou_period)
        
        # Get senkou span B
        senkou_span_b = self._senkou_span_b()
        non_shift_senkou_span_b = senkou_span_b.shift(-self.chikou_senkou_period)
        
        # Get chikou
        chikou = self._chikou()
        shifted_close = self.testing_dataframe['close'].shift(self.chikou_senkou_period)
        ...
        self.testing_dataframe["Signal / No Signal"] = np.where(
            np.logical_or(
                np.logical_and(
                    self.testing_dataframe['close']         > self.testing_dataframe['tenkan'],
                    self.testing_dataframe['tenkan']        > self.testing_dataframe['kijun'], 
                    self.testing_dataframe['kijun']         > self.testing_dataframe['senkou_span_a'], 
                    self.testing_dataframe['senkou_span_a'] > self.testing_dataframe['senkou_span_b'], 
                    non_shift_senkou_span_a                 > non_shift_senkou_span_b, 
                    self.testing_dataframe['close']         > shifted_close),
    ...
    def _chikou(self) -> pd.Series:
        ...
        chikou = self.testing_dataframe['close'].shift(-self.chikou_senkou_period)
        chikou.rename("chikou")
        
        return chikou
    
    def _senkou_span_a(self, tenkan: pd.Series, kijun: pd.Series) -> pd.Series:
        """
        ...
        senkou_span_a = ((tenkan + kijun) / 2).shift(self.chikou_senkou_period)
        senkou_span_a.rename("senkou_span_a")
        
        return senkou_span_a
    
    def _senkou_span_b(self) -> pd.Series:
        """
        ...
        # Get two copies of close candle Series
        senkou_span_b = ((lows + highs) / 2).shift(self.chikou_senkou_period)
        senkou_span_b.rename("senkou_span_b")
